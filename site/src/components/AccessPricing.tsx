    ...
    <section id="access" className="border-t border-white/5 px-5 py-24 md:px-12">
      ...
      >
        <motion.div variants={riseChild} className="font-mono-t mb-4 text-[11.5px] tracking-[0.18em] text-[var(--fg-dim)]">
          <span className="text-accent">{text.pricing.tag.split(" / ")[0]}</span> / {text.pricing.tag.split(" / ")[1]}
        </motion.div>
        ...
        <div className="mt-14 grid max-w-4xl items-start gap-5 md:grid-cols-12">
          <motion.div variants={riseChild} className="rounded-2xl border border-white/5 bg-[var(--bg-2)] p-9 md:col-span-5">
            <div className="font-mono-t mb-4 text-[12px] tracking-[0.14em] text-[var(--fg-dim)]">{free.plan}</div>
          ...
          <motion.div
            ...
            style={{ borderColor: "var(--accent-glow)", background: "linear-gradient(160deg, var(--bg-2) 60%, var(--accent-soft))" }}
          >
            <span className="font-mono-t text-accent absolute top-8 right-8 rounded-full border px-3 py-1 text-[10.5px] tracking-wider" style={{ borderColor: "var(--accent-glow)" }}>
              {pro.save}
            </span>
            <div className="font-mono-t mb-4 text-[12px] tracking-[0.14em] text-[var(--fg-dim)]">{pro.plan}</div>
