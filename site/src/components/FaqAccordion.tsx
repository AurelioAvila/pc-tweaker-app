    ...
    <section id="faq" className="border-t border-white/5 px-5 py-24 md:px-12">
      ...
      >
        <motion.div variants={riseChild} className="font-mono-t mb-4 text-[11.5px] tracking-[0.18em] text-[var(--fg-dim)]">
          <span className="text-accent">{text.faq.tag.split(" / ")[0]}</span> / {text.faq.tag.split(" / ")[1]}
        </motion.div>
            ...
            return (
              <div key={item.q} className="border-b border-white/5">
                ...
                >
                  {item.q}
                  <motion.span
                    animate={{ rotate: isOpen ? 45 : 0, color: isOpen ? "var(--accent)" : "var(--fg-dim)" }}
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <p className="max-w-2xl pb-6 text-[14.5px] leading-relaxed text-[var(--fg-dim)]">{item.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
